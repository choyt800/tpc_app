class StripeController < ApplicationController
  ...
  def webhook
    ...
    if event_type == "invoice.payment_succeeded"
      ...
      custom_sub = CustomSubscription.find_by(stripe_sub_id: event_subscription)
      mail_service = MailService.find_by(stripe_sub_id: event_subscription)

      if custom_sub
        custom_sub.update_attributes!(next_invoice_date: Time.at(event_period_end), invoice_amount: event_amount_due)
        puts "Updated custom subscription #{custom_sub.id} with Next Invoice Date of #{Time.at(event_period_end)} and Invoice Amount of #{event_amount_due}"
      elsif mail_service
        ...
        puts "No custom subscription or mail service found with Stripe Sub ID #{event_subscription}"
      end
    end
